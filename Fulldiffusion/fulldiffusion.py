...
def target(argin):
    logLevel, nRounds, nRows, nColumns, wordSize, nKolumns = argin
    rindaelTest = DiffusionTest(logLevel, nRounds, nRows, nColumns, wordSize,
                                nKolumns)
    return encryptionDiffusion(rindaelTest.encrypt)


def write2cvs(argin, argout, **kwargs):
    logLevel, nRounds, nRows, nColumns, wordSize, nKolumns = argin
    halfdiffusion = argout
    blockSize = nRows*nColumns*wordSize
    keySize = nRows*nKolumns*wordSize
    fileName = kwargs['fileName']
    lock = kwargs['lock']
    with lock:
        print("nRounds = %2d, nRows = %2d, nColumns = %2d, wordSize = %2d, "
              "nKeyColumns = %2d, blockSize = %4d, keySize = %4s -> "
              "full-diffusion = %2d"
              % (nRounds, nRows, nColumns, wordSize, nKolumns, blockSize,
                 keySize, halfdiffusion*2))
    ...
    if options.rijndael is not None:
                ...
                for wordSize in range(3, 17):
                    for nKolumns in range(2, 17):
                        if nKolumns >= nColumns:
                            ...
                            arginLst.append([options.log_level, nRounds, nRows,
                                             nColumns, wordSize, nKolumns])
        if options.processors is None or Pool is None:
            ...
            for argin in arginLst:
                argout = target(argin)
                write2cvs(argin, argout, **{'lock': lock,
                                            'fileName': fileName})
        else:
